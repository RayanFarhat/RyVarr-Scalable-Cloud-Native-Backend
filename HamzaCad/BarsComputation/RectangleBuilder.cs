...
namespace HamzaCad.BarsComputation
{
    public class RectangleBuilder
    {
        ...
        public static List<Rectangle> getRectangles(List<Line2D> lines, List<Line2D> Hlines, List<Point2D> vertices)
        {
            ...
            for (int i = 0; i < rectangles.Count; i++)
            {
                for (int j = 0; j < Hlines.Count; j++)
                {
                    ...
                    if (
                        DoubleUtils.IsEqual(Hlines[j].StartPoint.X, rectangles[i].Xleft) || DoubleUtils.IsEqual(Hlines[j].EndPoint.X, rectangles[i].Xright) ||
                        DoubleUtils.IsEqual(Hlines[j].StartPoint.X, rectangles[i].Xright) || DoubleUtils.IsEqual(Hlines[j].EndPoint.X, rectangles[i].Xleft)
                        )
                    {
                        //if line is inside the rect
                        if (rectangles[i].Yupper > Hlines[j].StartPoint.Y && Hlines[j].StartPoint.Y > rectangles[i].Ylower &&
                            (Hlines[j].EndPoint.X < rectangles[i].Xright || DoubleUtils.IsEqual(Hlines[j].EndPoint.X, rectangles[i].Xright))
                            )
                        {
        ...
        private static bool DoLinesMakeRectangle(Line2D line1, Line2D line2)
        {
            // Check if the lines are coincident (same Y coordinates for both endpoints).
            if (DoubleUtils.IsEqual(line1.StartPoint.Y, line2.StartPoint.Y) &&
                DoubleUtils.IsEqual(line1.EndPoint.Y, line2.EndPoint.Y))
            {
